...
class Agent(object):
    ...
    def choose_action(self, s):
        ...
        # print("State tensor shape:", s.shape)  # 添加这行来打印状态张量的形状
        return a.item()

    def push_data(self, transitions):
        # s, a, r, s_, done = transitions
    ...
    for _ in range(EP_MAX):
        ...
        while start:
            for i in range(HORIZON):
                env.render()
                s_npy = np.array([s], dtype=np.float32)
                a = agent.choose_action(torch.tensor(s_npy, dtype=torch.float))
